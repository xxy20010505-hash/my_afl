...
/* ========================================================================= */
/* CUSTOM ACTOR-LEARNER CODE START                                           */
/* (Inserted right before calculate_score to ensure types exist)             */
/* ========================================================================= */
...
static void init_redis(void) {
  ...
  if (redis_ctx == NULL || redis_ctx->err) {
    if (redis_ctx) {
      WARNF("Connection error: %s\n", redis_ctx->errstr);
      redisFree(redis_ctx);
    } else {
      WARNF("Connection error: can't allocate redis context\n");
    }
    redis_connected = 0;
    use_nn_scheduling = 0;
  } else {
    OKF("Connected to Redis for NN scheduling.");
    redis_connected = 1;
    /* 启动时清理旧队列，防止混淆 */
    redisReply *reply;
    reply = redisCommand(redis_ctx, "DEL train_queue"); if(reply) freeReplyObject(reply);
    reply = redisCommand(redis_ctx, "DEL global_model_version"); if(reply) freeReplyObject(reply);
  }
}

/* --- 2. 启动后台 Learner 进程 --- */
static void spawn_learner_process(void) {
  ACTF("Spawning background Learner process...");
  learner_pid = fork();
  if (learner_pid < 0) PFATAL("fork() failed");
  ...
  if (!learner_pid) {
    ...
    /* 重定向输出到 /dev/null，保持 AFL 界面整洁 */
    s32 null_fd = open("/dev/null", O_RDWR);
    dup2(null_fd, 1);
    dup2(null_fd, 2);
    close(null_fd);
...
/* --- 3. 初始化 ONNX Runtime --- */
static void init_onnx_env(void) {
  g_ort = OrtGetApiBase()->GetApi(ORT_API_VERSION);
  if (!g_ort) FATAL("Failed to init ONNX Runtime API");
  
  if (g_ort->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "afl_nn_env", &ort_env) != NULL)
    FATAL("Failed to create ONNX environment");
    
  if (g_ort->CreateSessionOptions(&ort_session_options) != NULL)
    FATAL("Failed to create session options");
    
  /* 【关键】禁用优化以防止段错误 */
  OrtStatus* status;

  status = g_ort->SetSessionGraphOptimizationLevel(ort_session_options, ORT_DISABLE_ALL);
  if (status) g_ort->ReleaseStatus(status);

  status = g_ort->DisableMemPattern(ort_session_options);
  if (status) g_ort->ReleaseStatus(status);

  status = g_ort->DisableCpuMemArena(ort_session_options);
  if (status) g_ort->ReleaseStatus(status);

  status = g_ort->SetIntraOpNumThreads(ort_session_options, 1);
  if (status) g_ort->ReleaseStatus(status);

  status = g_ort->SetInterOpNumThreads(ort_session_options, 1);
  if (status) g_ort->ReleaseStatus(status);
  
  if (g_ort->CreateCpuMemoryInfo(OrtArenaAllocator, OrtMemTypeDefault, &ort_memory_info) != NULL)
    FATAL("Failed to create memory info");
    
  OKF("ONNX Runtime initialized.");
}
...
static float predict_score(float* features) {
  ...
  size_t data_len = INPUT_DIM * sizeof(float);

  /* 封装输入 Tensor */
  if (g_ort->CreateTensorWithDataAsOrtValue(ort_memory_info, features, data_len, input_shape, 2, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &input_tensor) != NULL)
    return -1.0f;
  ...
  /* 执行推理 */
  if (g_ort->Run(ort_session, NULL, input_names, (const OrtValue* const*)&input_tensor, 1, output_names, 1, &output_tensor) != NULL) {
  ...
  OrtStatus* data_status = g_ort->GetTensorMutableData(output_tensor, (void**)&float_arr);
  if (data_status) {
...
static void handle_stop_sig(int sig) {
  ...
  /* ====== 清理 Learner 进程 ====== */
  if (learner_pid > 0) {
      kill(learner_pid, SIGTERM);
      waitpid(learner_pid, NULL, 0); 
  }
  /* ===================================== */
...
int main(int argc, char** argv) {
  ...
  /* 2. 启动 Python Learner */
  if (redis_connected) {
      spawn_learner_process();
      sleep(1); /* 等待 Python 启动 */
  }
...
stop_fuzzing:
  ...
  /* ====== 退出清理 ====== */
  if (learner_pid > 0) kill(learner_pid, SIGTERM);
